...
class DietaHombreGananciaPage extends StatefulWidget {
  const DietaHombreGananciaPage({super.key});

  @override
  State<DietaHombreGananciaPage> createState() => _DietaHombreGananciaPageState();
}

class _DietaHombreGananciaPageState extends State<DietaHombreGananciaPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final dieta = dietasGananciaHombre[_dietaSeleccionada];
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            // --- MODULO AGUA (MASCULINO: 15 vasos / 3.75L) ---
            ...
            // --- SELECTOR DE DIETAS ---
            Text('Variante de Dieta (~3000 kcal)', style: GoogleFonts.outfit(color: Colors.white70, fontSize: 16, fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            SingleChildScrollView(
              scrollDirection: Axis.horizontal,
              child: Row(
                children: List.generate(dietasGananciaHombre.length, (index) {
                  final d = dietasGananciaHombre[index];
            ...
            Row(
              ...
              children: [
                ...
                _buildMacroStat('Carbs', dieta.carbohidratos, Colors.orangeAccent),
            ...
            Text('Plan de Volumen Diario', style: GoogleFonts.outfit(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
                  ...
                  children: dieta.cronograma.map((comida) {
                    ...
                    return ExpansionTile(
                      ...
                      children: [
                        ...
                        Text('Porción exacta (Superávit Masivo):', style: TextStyle(color: Colors.white70, fontWeight: FontWeight.bold, fontSize: 13)),
                        const SizedBox(height: 4),
  ...
  Widget _buildMacroStat(String label, String value, Color clr) {
    return Column(
      children: [
        Text(value, style: GoogleFonts.outfit(color: clr, fontSize: 20, fontWeight: FontWeight.bold)),
  ...
  Widget _buildWaterModule(MetricsProvider metrics) {
    const meta = 15; // 15 vasos (~3.75L) para hombres
    final objAlcanzado = metrics.waterGlasses >= meta;

    return Card(
      color: Colors.blue.shade900.withOpacity(0.3),
      shape: RoundedRectangleBorder(side: BorderSide(color: Colors.blueAccent.withOpacity(0.5)), borderRadius: BorderRadius.circular(16)),
      child: Padding(
        padding: const EdgeInsets.all(16.0),
        child: Row(
          children: [
            Stack(
              ...
              children: [
                ...
                Icon(objAlcanzado ? Icons.verified : Icons.water_drop, color: Colors.lightBlueAccent, size: 30),
              ],
            ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text('Hidratación Diaria', style: GoogleFonts.outfit(color: Colors.lightBlueAccent, fontWeight: FontWeight.bold, fontSize: 18)),
                  Text('${metrics.waterGlasses} de $meta vasos (3.75L)', style: const TextStyle(color: Colors.white, fontSize: 16)),
                  const SizedBox(height: 8),
                  Row(
                    children: [
                      _roundedBtn(Icons.remove, () => metrics.removeWaterGlass(), Colors.blueGrey),
                      const SizedBox(width: 16),
                      _roundedBtn(Icons.add, () => metrics.addWaterGlass(), Colors.lightBlueAccent),
                    ],
                  ),
                ],
              ),
  ...
  Widget _buildWeightModule(MetricsProvider metrics) {
    ...
    if (curWeight != null && lstWeight != null) {
      ...
      if (diff > 0) {
        ...
        deltaClr = Colors.greenAccent; // En volumen subir de peso es POSITIVO
        deltaIco = Icons.trending_up;
      } else if (diff < 0) {
        deltaTxt = '${diff.toStringAsFixed(1)} kg';
        deltaClr = Colors.redAccent;   // Bajar de peso en volumen es ALERTA ROJA
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Control de Peso', style: GoogleFonts.outfit(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18)),
            const SizedBox(height: 16),
            Row(
              children: [
                Expanded(
                ...
                if (curWeight != null) ...[
                  ...
                  Column(
                    ...
                    children: [
                      Text('${curWeight.toStringAsFixed(1)} kg', style: GoogleFonts.outfit(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
                      if (curWeight != lstWeight)
                        Row(
                          children: [
                            Icon(deltaIco, color: deltaClr, size: 16),
                            const SizedBox(width: 4),
                            Text(deltaTxt, style: TextStyle(color: deltaClr, fontWeight: FontWeight.bold)),
                          ],
                        )
                    ],
                  ),
  ...
  Widget _roundedBtn(IconData ico, VoidCallback onTap, Color bg) {
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(shape: BoxShape.circle, color: bg.withOpacity(0.2), border: Border.all(color: bg)),
