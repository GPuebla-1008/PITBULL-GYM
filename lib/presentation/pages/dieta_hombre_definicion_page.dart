...
class DietaHombreDefinicionPage extends StatefulWidget {
  const DietaHombreDefinicionPage({super.key});

  @override
  State<DietaHombreDefinicionPage> createState() => _DietaHombreDefinicionPageState();
}

class _DietaHombreDefinicionPageState extends State<DietaHombreDefinicionPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final dieta = dietasDefinicionHombre[_dietaSeleccionada];
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // --- SELECTOR DE DIETAS ---
            Text('Variante de Dieta (2200 kcal)', style: GoogleFonts.outfit(color: Colors.white70, fontSize: 16, fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            SingleChildScrollView(
              scrollDirection: Axis.horizontal,
              child: Row(
                children: List.generate(dietasDefinicionHombre.length, (index) {
                  final d = dietasDefinicionHombre[index];
            ...
            Row(
              ...
              children: [
                ...
                _buildMacroStat('Carbs', dieta.carbohidratos, Colors.blueAccent),
            ...
            Text('Plan de Ingesta Diario', style: GoogleFonts.outfit(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
                  ...
                  children: dieta.cronograma.map((comida) {
                    ...
                    return ExpansionTile(
                      ...
                      children: [
                        ...
                        Text('Porción exacta (Macros Elevados):', style: TextStyle(color: Colors.white70, fontWeight: FontWeight.bold, fontSize: 13)),
                        const SizedBox(height: 4),
            ...
            // --- LISTA DE COMPRA ---
            Text('Mercado (Dosis Hombres)', style: GoogleFonts.outfit(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            Card(
              color: AppTheme.warmGrey,
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
  ...
  Widget _buildWaterModule(MetricsProvider metrics) {
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(side: BorderSide(color: Colors.tealAccent.withOpacity(0.5)), borderRadius: BorderRadius.circular(16)),
      child: Padding(
        padding: const EdgeInsets.all(16.0),
        child: Row(
          children: [
            Stack(
              ...
              children: [
                ...
                Icon(objAlcanzado ? Icons.verified : Icons.water_drop, color: Colors.tealAccent, size: 30),
              ],
            ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text('Hidratación Diaria', style: GoogleFonts.outfit(color: Colors.tealAccent, fontWeight: FontWeight.bold, fontSize: 18)),
                  Text('${metrics.waterGlasses} de $meta vasos (3.5L)', style: const TextStyle(color: Colors.white, fontSize: 16)),
                  const SizedBox(height: 8),
                  Row(
                    children: [
                      _roundedBtn(Icons.remove, () => metrics.removeWaterGlass(), Colors.blueGrey),
                      const SizedBox(width: 16),
                      _roundedBtn(Icons.add, () => metrics.addWaterGlass(), Colors.tealAccent),
                    ],
                  ),
                ],
              ),
